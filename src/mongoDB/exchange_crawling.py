...
class exchange():
    def __init__(self):
        self.driver = webdriver.Chrome('/Users/yuseonjong/chromedriver')
            
    def country(self, country_name, date=None):
        self.driver.get('https://google.com')
        self.driver.maximize_window()
        self.driver.execute_script('window.open("https://spot.wooribank.com/pot/Dream?withyou=FXXRT0014");')
        time.sleep(3)

        self.driver.switch_to.window(self.driver.window_handles[-1])

        #날짜 선택(제일 처음)
        select_year = Select(self.driver.find_element_by_id('START_DATEY'))
        select_month = Select(self.driver.find_element_by_id('START_DATEM'))
        select_day = Select(self.driver.find_element_by_id('START_DATED'))
        if not date:
        ...
        # 국가 선택
        select = Select(self.driver.find_element_by_id('id01')) # Select 함수를 이용하여 드롭 다운 메뉴에서 요소 선택
        ...
        self.driver.find_element_by_xpath('//*[@id="frm"]/fieldset/div/span/input').click() # 조회 클릭
        print('조회 선택했슴둥')
        time.sleep(30)

        # country_name = self.driver.find_element_by_xpath('//*[@id="fxprint"]/div/div/dl/dd[2]').text # 국가명 추출
        try:
